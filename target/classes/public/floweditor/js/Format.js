...
Format.prototype.init = function()
{
	...
	{
		//console.log(evt.properties.removed);
		this.clearSelectionState();
		this.refresh();
		// 点击节点刷新属性面板
		if (evt.name == 'change' && evt.properties && evt.properties.removed) {
			var cells = evt.properties.removed;
			if (!isNull(cells) && cells.length==1) {
				// 将选中节点存入公共变量
				nowSelectNode = cells[0];
				// console.log(nowSelectNode);
				// 刷新业务属性面板
				if (control_logic_op == 1) {
					this.editorUi.logic.Refresh(cells[0]);
				}
				// 刷新自适应设计面板
				if (control_adaptive_op == 1) {
					this.editorUi.adaptiveDesign.Refresh(cells[0]);
				}
			}
		}
		// 未选中节点时操作
		if (graph.isSelectionEmpty()) {
			...
			// 自适应设计面板刷新
			if (control_adaptive_op == 1) {
				this.editorUi.adaptiveDesign.Refresh(null);
			}
		}
		// 数据编辑状态改为修改中
		if (evt.name == 'change' && evt.properties && evt.properties.changes) {
			nowFlowSaveOp = 1;
...
Format.prototype.refresh = function()
{
  ...
  i.setAttribute('class', 'fas fa-puzzle-piece');
  ...
  tittleDiv.style.top = ((control_menu_op==1?ui.menubarHeight:0)+(control_toolbar_op==1?ui.toolbarHeight:0))+'px';
  tittleDiv.appendChild(i);
  mxUtils.write(tittleDiv, '样式设置');
  this.container.appendChild(tittleDiv);
	
  // 新增tab标签专用样式(在原有样式基础上增加宽度和fixed)
  ...
  tabDiv.style.width = '241px';
  tabDiv.style.display = 'block';
  tabDiv.style.position = 'fixed';
  tabDiv.style.paddingTop = '34px';
  tabDiv.style.zIndex = 1;
	tabDiv.style.top = ((control_menu_op==1?ui.menubarHeight:0)+(control_toolbar_op==1?ui.toolbarHeight:0))+'px';
	this.container.appendChild(tabDiv);

  // 原始样式(在原有样式基础上增加宽度和paddingTop)
  ...
  div.style.width = '241px';
    ...
    label.style.backgroundColor = '#fbfbfb';
...
BaseFormatPanel.prototype.createPanel = function()
{
	...
	div.style.padding = '5px 0px 0 12px';
	...
	var apply = function(color, disableUpdate, forceUpdate)
	{
		if (!applying)
		{
			...
			btn.innerHTML = '<div style="width:' + ((mxClient.IS_QUIRKS) ? '30' : '36') +
				'px;height:8px;margin:3px;border:1px solid black;background-color:' +
 ...
 * 
 */
BaseFormatPanel.prototype.addUnitInput = function(container, unit, right, width, update, step, marginTop, disableFocus, isFloat, LeftMarginTop)
{
	...
	input.style.marginTop = ((LeftMarginTop != null && LeftMarginTop != 'undefined' ? LeftMarginTop : 0) - 2) + 'px';
...
BaseFormatPanel.prototype.createRelativeOption = function(label, key, width, handler, init)
{
	...
	var input = this.addUnitInput(div, '%', 20, width, update, 10, -15, handler != null, null, 1);
...
TextFormatPanel.prototype.addFont = function(container)
{
	...
	stylePanel.style.paddingTop = '0';
	stylePanel.style.paddingBottom = '0';
	stylePanel.style.position = 'relative';
	...
	stylePanel4.style.paddingTop = '2px';
	...
	extraPanel.style.paddingBottom = '0';
	...
	spacingPanel.style.paddingTop = '5px';
	spacingPanel.style.paddingBottom = '24px';
  spacingPanel.style.fontWeight = 'normal';
  spacingPanel.style.borderBottom = 0;
    ...
    wrapper3.style.borderBottom = 0;
    wrapper3.style.borderTop = '1px solid #dadce0';
...
/**
 * Adds the label menu items to the given menu and parent. 二次开发
 */
StyleFormatPanel.prototype.init = function()
{
	...
	if (!ss.containsLabel)
	{
		...
		opacityPanel.style.paddingTop = '5px';
    opacityPanel.style.paddingBottom = '2px';
    opacityPanel.style.borderBottom = 0;
...
StyleFormatPanel.prototype.addStroke = function(container)
{
	...
	stylePanel.style.marginBottom = '0';
	stylePanel.style.marginTop = '0';
	stylePanel.className = 'geToolbarContainer';
	...
	perimeterPanel.style.marginBottom = '0';
	perimeterPanel.style.marginTop = '2px';
...
DiagramFormatPanel.prototype.init = function()
{
	...
	if (graph.isEnabled())
	{
		this.container.appendChild(this.addOptions(this.createPanel())); // 选项
		//this.container.appendChild(this.addPaperSize(this.createPanel())); // 纸张大小
		this.container.appendChild(this.addStyleOps(this.createPanel())); // 样式
	}
};
