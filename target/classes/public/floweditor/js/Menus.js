 ...
 */
Menus.prototype.defaultFont = 'Arial';
...
if (function_open_adaptive_op == 1) {
	Menus.prototype.defaultMenuItems = ['model', 'modelData'];
}
if (function_open_basic_op == 1 && function_open_adaptive_op == 1) {
	Menus.prototype.defaultMenuItems = ['file', 'edit', 'view', 'arrange', 'model'];
}
if (function_open_course_op == 1) {
	Menus.prototype.defaultMenuItems = ['courseData'];
}
/**
 * Adds the label menu items to the given menu and parent.
 */
Menus.prototype.defaultFonts = ['Arial', 'Helvetica', 'sans-serif', '宋体', '黑体',
           		             '华文仿宋', '楷体', '隶书'];
...
Menus.prototype.init = function()
{
	...
	this.put('arrange', new Menu(mxUtils.bind(this, function(menu, parent) // 排列 菜单
	{
		...
		// this.addSubmenu('insert', menu, parent); // 二次修改 排列 -> 插入
		this.addSubmenu('layout', menu, parent);
		this.addMenuItems(menu, ['-', 'group', 'ungroup', 'removeFromGroup', '-', 'clearWaypoints', 'autosize'], parent);
	}))).isEnabled = isGraphEnabled;
	this.put('insert', new Menu(mxUtils.bind(this, function(menu, parent) // 二次修改 快捷工具栏 -> 十字加号[链接,图片] 菜单
	{
    ...
    this.addMenuItems(menu, ['-', 'fitWindow', 'fitPageWidth', '-', 'customZoom'], parent);
	})));
    ...
    //this.addMenuItems(menu, ['new', 'open', '-', 'save', 'saveAs', '-', 'import', 'export', '-', 'pageSetup', 'print'], parent);
		...
		tmpMenus.push('new');
		tmpMenus.push('open');
		tmpMenus.push('-');tmpMenus.push('save');tmpMenus.push('saveEdit');tmpMenus.push('saveAs');
		tmpMenus.push('-');tmpMenus.push('submit');tmpMenus.push('-');tmpMenus.push('script');
		tmpMenus.push('-');tmpMenus.push('exam');tmpMenus.push();tmpMenus.push('workTime');
    this.addMenuItems(menu, tmpMenus, parent);
	})));
	...
	// this.put('extras', new Menu(mxUtils.bind(this, function(menu, parent) // 二次修改 扩展菜单
	// {
	// 	this.addMenuItems(menu, ['copyConnect', 'collapseExpand', '-', 'editDiagram']);
	// })));
	// this.put('help', new Menu(mxUtils.bind(this, function(menu, parent) // 二次修改 帮助菜单
	// {
	...
	{
    	var tmpMenus = [];
		tmpMenus.push('openModel');
		tmpMenus.push('stepCount');
    	this.addMenuItems(menu, tmpMenus, parent);
	})));
	this.put('modelData', new Menu(mxUtils.bind(this, function(menu, parent)
	{
    	var tmpMenus = [];
		tmpMenus.push('modelDataOpen');
		tmpMenus.push('modelDataSave');
		tmpMenus.push('modelDataSaveEdit');
		tmpMenus.push('modelDataSaveAs');
    	this.addMenuItems(menu, tmpMenus, parent);
	})));
	this.put('courseData', new Menu(mxUtils.bind(this, function(menu, parent)
	{
    	var tmpMenus = [];
		tmpMenus.push('courseDataNew');
		tmpMenus.push('courseDataOpen');
		tmpMenus.push('courseDataSave');
		tmpMenus.push('courseDataSaveEdit');
		tmpMenus.push('courseDataSaveAs');
    	this.addMenuItems(menu, tmpMenus, parent);
	})));
};
...
Menus.prototype.addInsertTableItem = function(menu)
{
	...
	function createTable(rows, cols)
	{
		...
		for (var i = 0; i < rows; i++)
		{
			...
			{
				html.push('<td border="1" style="border: 1px solid rgb(0, 0, 0); border-collapse: collapse;"><br></td>');
			}
