  ...
  this.actions = new Actions(this);
	...
	if (!graph.standalone)
	{
		...
		if (this.container == document.body)
		{
			...
			this.outlineContainer.onselectstart = textEditing;
			this.outlineContainer.onmousedown = textEditing;
			this.formatContainer.onselectstart = textEditing;
			this.formatContainer.onmousedown = textEditing;
			this.logicContainer.onselectstart = textEditing;
			this.logicContainer.onmousedown = textEditing;
			this.adaptiveDesignContainer.onselectstart = textEditing;
			this.adaptiveDesignContainer.onmousedown = textEditing;
    ...
    // 节点创建事件
       ...
       * 自定义业务属性相关代码
       */
      // if (!isNull(cells) && cells.length==1) {
      //   myCreateUI.logic.Refresh(cells[0]);
      // }
		});
...
/**
 * Specifies the height of the footer. Default is 28. 页脚高度 因为暂时没发现用途所以隐藏
 */
...
EditorUi.prototype.init = function()
{
  var myCreateUI = this;
    ...
    //   if (!isNull(cell)) {
    //     myCreateUI.logic.Refresh(cell);
    //   } else {
    //     myCreateUI.logic.Clear();
    //   }
    // });
...
EditorUi.prototype.isSelectionAllowed = function(evt)
{
  var nodeName = mxEvent.getSource(evt).nodeName;
	return nodeName == 'SELECT' || (nodeName == 'INPUT' &&
...
EditorUi.prototype.updateActionStates = function()
{
	...
	if (function_draw_edit_op == 1) { // 绘图区可编辑
...
EditorUi.prototype.refresh = function(sizeDidChange)
{
	...
	{
		if (control_menu_op == 1 && control_toolbar_op == 0) {
      this.menubarContainer.style.borderBottom = '1px solid #dadce0'; // 单独显示菜单时补条线
    } else {
      this.menubarContainer.style.borderBottom = 0; // 去掉补线
    }


    if (control_menu_op == 1) {
      this.menubarContainer.style.height = this.menubarHeight + 'px';
      this.menubarContainer.style.display = '';
      tmp += this.menubarHeight;
    } else {
      this.menubarContainer.style.height = 0;
      this.menubarContainer.style.display = 'none'; // 屏蔽菜单
    }
	}
    ...
    if (control_toolbar_op == 1) {
      this.toolbarContainer.style.height = this.toolbarHeight + 'px';
      this.toolbarContainer.style.display = '';
      tmp += this.toolbarHeight;
    } else {
      this.toolbarContainer.style.height = 0;
      this.toolbarContainer.style.display = 'none'; // 屏蔽工具栏
    }
	}
	...
	if (control_sidebar_op == 1) {
    this.sidebarContainer.style.width = effHsplitPosition + 'px';
    this.sidebarContainer.style.display = '';
  } else {
    this.sidebarContainer.style.width = 0;
    this.sidebarContainer.style.display = 'none'; // 屏蔽节点面板
  }
  if (control_outline_op == 1) {
    this.outlineContainer.style.width = effHsplitPosition + 'px';
    this.outlineContainer.style.display = '';
  } else {
    this.outlineContainer.style.width = 0;
    this.outlineContainer.style.display = 'none'; // 屏蔽缩略图面板
  }
  this.formatContainer.style.top = tmp + 56 + 'px';
  if (control_format_op == 1) {
    this.formatContainer.style.width = fw + 'px';
    this.formatContainer.style.display = '';
  } else {
    this.formatContainer.style.width = 0;
    this.formatContainer.style.display = 'none'; // 屏蔽节点编辑面板
  }
	//this.formatContainer.style.display = (this.format != null) ? '' : 'none';
  this.formatContainer.style.height = '110px';
  if (control_format_op === 1 && control_logic_op ===0 && control_adaptive_op === 0) {
	this.formatContainer.style.height = '400px';
  }
  this.logicContainer.style.width = fw + 'px';
  this.adaptiveDesignContainer.style.width = fw + 'px';
  if (control_logic_op == 1) { // 业务属性面板
		// 显示业务属性面板
		var h = 0;
		if (control_menu_op == 1) h = h + 31;
		if (control_toolbar_op == 1) h = h + 39;
		if (control_format_op == 1) h = h + 165;
    this.logicContainer.style.height = 'calc(100% - ' + h + 'px)';
    this.logicContainer.style.top = h + 'px';
    this.logicContainer.style.display = '';
		// 隐藏自适应设计面板
    this.adaptiveDesignContainer.style.height = 0;
    this.adaptiveDesignContainer.style.display = 'none';
  } else if (control_adaptive_op == 1) { // 自适应设计面板
		// 隐藏业务属性面板
    this.logicContainer.style.height = 0;
    this.logicContainer.style.display = 'none';
		// 打开自适应设计面板
		var h = 0;
		if (control_menu_op == 1) h = h + 31;
		if (control_toolbar_op == 1) h = h + 39;
		if (control_format_op == 1) h = h + 165;
		this.adaptiveDesignContainer.style.height = 'calc(100% - ' + h + 'px)';
    this.adaptiveDesignContainer.style.top = h + 'px';
		this.adaptiveDesignContainer.style.display = '';
	}
  ...
  if (control_sidebar_op == 1) { // 有左侧节点工具栏时 编辑区域的样式
    this.diagramContainer.style.left =  (contLeft + diagContOffset.x) + 'px';
  } else { // 无左侧工具栏时 编辑区域的样式
  ...
  if (control_sidebar_op == 1) { // 有左侧节点工具栏时 显示拖动工具条
    this.hsplit.style.left = effHsplitPosition + 'px';
    this.hsplit.style.width = this.splitSize + 'px';
    this.hsplit.style.display = '';
  } else {
    this.hsplit.style.left = 0;
    this.hsplit.style.width = 0;
    this.hsplit.style.display = 'none'; // 屏蔽拖拽条
  }
	...
	else
	{
		...
		if (control_outline_op == 1) {
			this.sidebarContainer.style.bottom = (this.footerHeight + sidebarFooterHeight + off) + 150 + 'px';
		} else {
			this.sidebarContainer.style.bottom = (this.footerHeight + sidebarFooterHeight + off) + 'px';
		}
		this.outlineContainer.style.bottom = (this.footerHeight + sidebarFooterHeight + off) + 'px';
		this.logicContainer.style.bottom = (this.footerHeight + off) + 'px';
		this.adaptiveDesignContainer.style.bottom = (this.footerHeight + off) + 'px';
...
EditorUi.prototype.createDivs = function()
{
  ...
  this.logicContainer = this.createDiv('geLogicContainer');
  this.adaptiveDesignContainer = this.createDiv('geAdaptiveDesignContainer');
  ...
  // 缩略图高度
  this.outlineContainer.style.height = '150px';
  ...
  this.outlineContainer.style.left = '0px';
	...
	this.logicContainer.style.right = '0px';
	this.logicContainer.style.zIndex = '1';
	this.adaptiveDesignContainer.style.right = '0px';
	this.adaptiveDesignContainer.style.zIndex = '1';
...
EditorUi.prototype.createUi = function()
{
  ...
  // 创建缩略图
	if (control_outline_op == 1) {
		this.outline = (this.editor.chromeless) ? null : this.createOutlineBar(this.outlineContainer);
	...
	if (control_format_op == 1 || control_logic_op == 1 || control_adaptive_op == 1) {
	...
	if (control_logic_op == 1) {
		this.logic = (this.editor.chromeless) ? null : this.createLogic(this.logicContainer);
		
		if (this.logic != null)
		{
			this.container.appendChild(this.logicContainer);
		}
	}
	
	// 创建自适应设计面板
	if (control_adaptive_op == 1) {
		this.adaptiveDesign = (this.editor.chromeless) ? null : this.createAdaptiveDesign(this.adaptiveDesignContainer);
		
		if (this.adaptiveDesign != null)
		{
			this.container.appendChild(this.adaptiveDesignContainer);
		}
	}
 ...
 */
if (control_outline_op == 1) {
	EditorUi.prototype.createOutlineBar = function(container)
	{
		return new Outline(this, container);
	};
}
 ...
 */
if (control_logic_op == 1) {
	EditorUi.prototype.createLogic = function(container)
	{
		return new Logic(this, container);
	};
}
 ...
 */
if (control_adaptive_op == 1) {
	EditorUi.prototype.createAdaptiveDesign = function(container)
	{
		return new AdaptiveDesign(this, container);
	};
}
...
EditorUi.prototype.destroy = function()
{
  ...
  var c = [this.menubarContainer, this.toolbarContainer, this.sidebarContainer,
           ...
           this.logicContainer, this.adaptiveDesignContainer,
