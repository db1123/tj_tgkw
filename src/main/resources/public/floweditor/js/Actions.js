...
Actions.prototype.init = function()
{
	...
	// 自定义
	myActionsBasics(this);
	myActionsAdaptive(this);
	myActionsCourse(this);
	// File actions
  ...
  //this.addAction('saveAs...', function() { ui.saveFile(true); }, null, null, Editor.ctrlKey + '+Shift+S').isEnabled = isGraphEnabled;
	...
	// Edit actions
	this.addAction('undo', function() { if(function_undo_redo_op==1)ui.undo(); }, null, 'sprite-undo', Editor.ctrlKey + '+Z');
	this.addAction('redo', function() { if(function_undo_redo_op==1)ui.redo(); }, null, 'sprite-redo', (!mxClient.IS_WIN) ? Editor.ctrlKey + '+Shift+Z' : Editor.ctrlKey + '+Y');
	this.addAction('cut', function() { if(function_draw_edit_op==1)mxClipboard.cut(graph); }, null, 'sprite-cut', Editor.ctrlKey + '+X');
	this.addAction('copy', function()
	{
		if(function_draw_edit_op==1) {
			try
			{
	...
	{
		if(function_draw_edit_op==1)deleteCells(evt != null && mxEvent.isShiftDown(evt));
	}, null, null, 'Delete');
	this.addAction('deleteAll', function()
	{
		if(function_draw_edit_op==1)deleteCells(true);
	...
	{
		if(function_draw_edit_op==1)graph.turnShapes(graph.getSelectionCells());
	}, null, null, Editor.ctrlKey + '+R'));
	this.addAction('selectVertices', function() { if(function_draw_edit_op==1)graph.selectVertices(null, true); }, null, null, Editor.ctrlKey + '+Shift+I');
	this.addAction('selectEdges', function() { if(function_draw_edit_op==1)graph.selectEdges(); }, null, null, Editor.ctrlKey + '+Shift+E');
	this.addAction('selectAll', function() { if(function_draw_edit_op==1)graph.selectAll(null, true); }, null, null, Editor.ctrlKey + '+A');
	this.addAction('selectNone', function() { if(function_draw_edit_op==1)graph.clearSelection(); }, null, null, Editor.ctrlKey + '+Shift+A');
	...
	// Navigation actions
	this.addAction('home', function() { if(function_draw_edit_op==1)graph.home(); }, null, null, 'Home');
	this.addAction('exitGroup', function() { if(function_draw_edit_op==1)graph.exitGroup(); }, null, null, Editor.ctrlKey + '+Shift+Home');
	this.addAction('enterGroup', function() { if(function_draw_edit_op==1)graph.enterGroup(); }, null, null, Editor.ctrlKey + '+Shift+End');
	this.addAction('collapse', function() { if(function_draw_edit_op==1)graph.foldCells(true); }, null, null, Editor.ctrlKey + '+Home');
	this.addAction('expand', function() { if(function_draw_edit_op==1)graph.foldCells(false); }, null, null, Editor.ctrlKey + '+End');
	
	// Arrange actions
	this.addAction('toFront', function() { if(function_draw_edit_op==1)graph.orderCells(false); }, null, null, Editor.ctrlKey + '+Shift+F');
	this.addAction('toBack', function() { if(function_draw_edit_op==1)graph.orderCells(true); }, null, null, Editor.ctrlKey + '+Shift+B');
	...
	this.addAction('removeFromGroup', function() { if(function_draw_edit_op==1)graph.removeCellsFromParent(); });
	// Adds action
  ...
  // 弹出对话框样例
	this.put('customZoom', new Action(mxResources.get('custom') + '...', mxUtils.bind(this, function()
	{
		...
		this.editorUi.showDialog(dlg.container, 300, 90, true, true);
		dlg.init();
	...
	toggleFontStyle('bold', mxConstants.FONT_BOLD, function() { if(function_undo_redo_op==1)document.execCommand('bold', false, null); }, Editor.ctrlKey + '+B');
	toggleFontStyle('italic', mxConstants.FONT_ITALIC, function() { if(function_undo_redo_op==1)document.execCommand('italic', false, null); }, Editor.ctrlKey + '+I');
	toggleFontStyle('underline', mxConstants.FONT_UNDERLINE, function() { if(function_undo_redo_op==1)document.execCommand('underline', false, null); }, Editor.ctrlKey + '+U');
	
	// Color actions
	this.addAction('fontColor...', function() { if(function_undo_redo_op==1)ui.menus.pickColor(mxConstants.STYLE_FONTCOLOR, 'forecolor', '000000'); });
	this.addAction('strokeColor...', function() { if(function_undo_redo_op==1)ui.menus.pickColor(mxConstants.STYLE_STROKECOLOR); });
	this.addAction('fillColor...', function() { if(function_undo_redo_op==1)ui.menus.pickColor(mxConstants.STYLE_FILLCOLOR); });
	this.addAction('gradientColor...', function() { if(function_undo_redo_op==1)ui.menus.pickColor(mxConstants.STYLE_GRADIENTCOLOR); });
	this.addAction('backgroundColor...', function() { if(function_undo_redo_op==1)ui.menus.pickColor(mxConstants.STYLE_LABEL_BACKGROUNDCOLOR, 'backcolor'); });
	this.addAction('borderColor...', function() { if(function_undo_redo_op==1)ui.menus.pickColor(mxConstants.STYLE_LABEL_BORDERCOLOR); });
	
	// Format actions
	this.addAction('vertical', function() { if(function_undo_redo_op==1)ui.menus.toggleStyle(mxConstants.STYLE_HORIZONTAL, true); });
	this.addAction('shadow', function() { if(function_undo_redo_op==1)ui.menus.toggleStyle(mxConstants.STYLE_SHADOW); });
	this.addAction('solid', function()
	{
		if (function_undo_redo_op == 1) {
			graph.getModel().beginUpdate();
			try
			{
  ...
  action = this.addAction('view', mxUtils.bind(this, function()
	{
		ui.toggleFormatPanel();
	}), null, null, Editor.ctrlKey + '+Shift+V');
	action.setToggleAction(true);
	action.setSelectedCallback(mxUtils.bind(this, function() { return ui.formatWidth > 0; }));
	action = this.addAction('outline', mxUtils.bind(this, function() // 缩略图
	{
		if (this.outlineWindow == null)
		{
			// LATER: Check layers window for initial placement
			this.outlineWindow = new OutlineWindow(ui, document.body.offsetWidth - 425, 75, 160, 180);
...
Actions.prototype.addAction = function(key, funct, enabled, iconCls, shortcut)
{
  ...
  var temp = new Action(title, funct, enabled, iconCls, shortcut);
	return this.put(key, temp);
};
